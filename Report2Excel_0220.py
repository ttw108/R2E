...
df = pd.DataFrame({'Strategy': np.arange(1,w.shape[0]+1),
                   'Weight': wx })
# 使用seaborn.barplot()繪製條形圖
plt.figure(figsize=(10, 8))
...
# 過濾出10天前的檔案和最舊的檔案
files_10_days_ago = [f for f in matching_files if datetime.datetime.fromtimestamp(os.path.getctime(f)) < ten_days_ago]
...
sns.lineplot(x=m7_df.index, y='cum_Sum', data=p7_df,linestyle="-",color="blue",label="MP_MMSE-{}".format(mpmmse7lis))
...
plt.legend(loc='upper left', borderaxespad=0., fontsize='large',fancybox=True, edgecolor='navy', framealpha=0.2, handlelength=1.5, handletextpad=0.5, borderpad=0.5, labelspacing=0.5)
#SQN
...
'''
sns.lineplot(x=dfak2.index, y='cumsum', data=dfak2,linestyle="--",color="orange",label="Last_Better-{}".format(lis_sqn_old))
plt.tight_layout()
