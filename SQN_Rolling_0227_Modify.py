...
def sqn_d1_cd():
    ...
    sqn_a1_best = sqn_a1_best[sqn_a1_best['a1'] > 0.01]
    ...
    sqn_cd_wsun_best = sqn_cd_wsun_best[sqn_cd_wsun_best['cd_sum'] > 0.01]
...
def corr_f():
    # 以下是針對關連性做排序
    # 針對策略做相關性測試
    global dfb,dfaa,dfab,dfac,dfad, dfaa_cumsum
    dfaa=dfa.iloc[-90:,:]
...
#dfaa##########################################################
pca_f(dfaa,sqn_a1_lis1,sqn_a1_lis1)
plt.tight_layout()
# plt.show()
...
################################################################
pca_f(dfab,sqn_a1_lis1,sqn_a1_lis1)
plt.tight_layout()
plt.savefig('temp.png')
plt.close()
pic_path = (os.path.join(os.getcwd(), "temp.png"))
...
pca_f(dfac,sqn_a1_lis1,sqn_a1_lis1)
plt.tight_layout()
plt.savefig('temp.png')
plt.close()
pic_path = (os.path.join(os.getcwd(), "temp.png"))
...
def dfaa_cumsum_():
    ...
    # dfaa CumSum ##########################################################
    pca_f(dfaa_cumsum, sqn_a1_lis1, sqn_a1_lis1)
    plt.tight_layout()
    # plt.show()
    plt.savefig('temp.png')
    plt.close()
